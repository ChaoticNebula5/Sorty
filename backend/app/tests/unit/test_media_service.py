...
def test_get_event_media_summary_counts_media_quality_and_review_rows() -> None:
    ...
    with Session() as db:
        ...
        db.add_all([processed, blurry, duplicate, failed, other])
        db.flush()
        db.add_all(
            [
        ...
        for media, status in (
            ...
            db.execute(
                text(
                    "insert into review_decisions (id, media_id, status) "
                    "values (:id, :media_id, :status)"
                ),
                {
                    "id": uuid.uuid4().hex,
    ...
    assert summary.processed_media == 1
